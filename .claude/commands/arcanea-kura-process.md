---
name: arcanea-kura-process
description: Arcanea-specific specialization of /arcanea-kura-process. Walks the local Kura/ vault, extracts characters / locations / artifacts / lore from each conversation, populates frontmatter, and emits Obsidian-linked entity notes under _entities/. Idempotent. Never overwrites canon. For generic markdown processing without worldbuilding entity extraction, use /arcanea-kura-process instead.
allowed-tools: Read, Write, Edit, Glob, Grep, Bash
---

# /arcanea-kura-process

You are the **Arcanea specialization** layer on top of a Kura vault.
Kura is the sovereign export format (see `/kura-process` for generic
markdown processing). This skill adds Arcanea's worldbuilding lens:
extracting characters, locations, artifacts, and lore so storytellers
get a linked entity graph from their AI conversations.

The browser extension has already done the capture — `~/Downloads/Kura/`
(or the path the user names) is a folder of platform-keyed conversation
notes written in Obsidian-compatible Markdown per `FORMAT_SPEC.md` v0.2.0.

Your job: turn raw captures into a **linked worldbuilding knowledge
graph** that Obsidian's graph view (or any wikilink-aware tool) can
render. You add entity links to frontmatter and emit per-entity notes.
You never invent content; every claim traces back to a verbatim source
line in the captured conversations.
...
4. Body (regenerated each run unless canon):
   - `# <Name>`
   - A one-line disclaimer: *"Auto-extracted by /arcanea-kura-process. Promote
...
```
/arcanea-kura-process
/arcanea-kura-process ~/Documents/Brain/Kura
/arcanea-kura-process "C:\Users\frank\Downloads\Kura"
```
